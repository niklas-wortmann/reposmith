# RepoAtlas — Product Requirements Document

## One-Line Summary

RepoAtlas scans any codebase, builds a dependency graph, generates agent-readable documentation, and scores how ready the repository is for AI coding agents.
...
A developer runs `npx repoatlas init` and learns something new about their repo within 10 seconds.

---

## Core Features (MVP)

### Feature 1: Repo Analysis (`repoatlas init`)
...
Created:
  ...
  .repoatlas/architecture.md       (module map and boundaries)
  .repoatlas/modules.md            (per-module documentation)
  .repoatlas/conventions.md        (detected golden principles)
  .repoatlas/entrypoints.md        (key entry points and APIs)
  .repoatlas/dependency-graph.json (machine-readable graph)
  .repoatlas/score.json            (health score breakdown)
```
...
This is the most important output. A short (~100 line) file in the repo root that serves as a table of contents pointing into `.repoatlas/`. Every major AI coding agent (Claude Code, Codex, Cursor, Copilot) automatically reads instruction files from the root. This file should:

- Summarize the project (purpose, stack, structure)
- List module boundaries with one-line descriptions
- Link to detailed docs in `.repoatlas/`
...
```
npx repoatlas scan github.com/vercel/next.js
```
...
View full report: https://repoatlas.dev/scores/vercel/next.js
```
...
| Command | Description |
|---------|-------------|
| `repoatlas init` | Analyze repo, generate .repoatlas/ docs, produce score |
| `repoatlas score` | Print health report (re-scans, no doc generation) |
| `repoatlas graph` | Output dependency graph (ASCII + JSON) |
| `repoatlas scan <github-url>` | Score a public GitHub repo remotely |
...
RepoAtlas is technology-agnostic. It must work on any codebase in any language. The architecture achieves this through four layers:
...
- "Show HN: RepoAtlas — Make your codebase readable for AI agents"
...
- Before/after agent performance anecdote (same prompt, with and without RepoAtlas)
