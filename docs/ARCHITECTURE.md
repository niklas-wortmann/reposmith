...
RepoForge is a CLI tool written in TypeScript that runs on Node.js. It analyzes codebases through four layered analysis stages, produces a universal repo graph, generates agent-readable documentation, and calculates a health score.
...
│                   Output Generators                      │
│                                                          │
│  AGENTS.md  |  .repoforge/*  |  score.json  |  ASCII  │
└─────────────────────────────────────────────────────────┘
```
...
│   │   ├── architecture-md.ts    # Generates .repoforge/architecture.md
│   │   ├── modules-md.ts         # Generates .repoforge/modules.md
│   │   ├── conventions-md.ts     # Generates .repoforge/conventions.md
│   │   ├── entrypoints-md.ts     # Generates .repoforge/entrypoints.md
│   │   ├── repo-summary-md.ts    # Generates .repoforge/repo-summary.md
│   │   ├── graph-json.ts         # Generates .repoforge/dependency-graph.json
│   │   ├── score-json.ts         # Generates .repoforge/score.json
...
To build edges (dependencies between modules), RepoForge must resolve import statements to modules.
...
The fallback is less accurate but ensures RepoForge produces useful output for any language.
...
Enable `repoforge scan <github-url>` to score public repos without cloning.
...
> Generated by [RepoForge](https://github.com/repoforge/repoforge).
...
For detailed module documentation, see [.repoforge/modules.md](.repoforge/modules.md).
For dependency graph, see [.repoforge/dependency-graph.json](.repoforge/dependency-graph.json).
...
For full conventions documentation, see [.repoforge/conventions.md](.repoforge/conventions.md).
...
RepoForge should never crash on unexpected input. If a file can't be parsed, skip it and log a warning. If a language isn't supported, fall back to regex. If the repo structure is unusual, do best-effort analysis and note limitations in the output.
