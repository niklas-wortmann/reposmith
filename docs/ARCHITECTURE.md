...
RepoAtlas is a CLI tool written in TypeScript that runs on Node.js. It analyzes codebases through four layered analysis stages, produces a universal repo graph, generates agent-readable documentation, and calculates a health score.
...
│                   Output Generators                      │
│                                                          │
│  AGENTS.md  |  .repoatlas/*  |  score.json  |  ASCII  │
└─────────────────────────────────────────────────────────┘
```
...
│   │   ├── architecture-md.ts    # Generates .repoatlas/architecture.md
│   │   ├── modules-md.ts         # Generates .repoatlas/modules.md
│   │   ├── conventions-md.ts     # Generates .repoatlas/conventions.md
│   │   ├── entrypoints-md.ts     # Generates .repoatlas/entrypoints.md
│   │   ├── repo-summary-md.ts    # Generates .repoatlas/repo-summary.md
│   │   ├── graph-json.ts         # Generates .repoatlas/dependency-graph.json
│   │   ├── score-json.ts         # Generates .repoatlas/score.json
...
To build edges (dependencies between modules), RepoAtlas must resolve import statements to modules.
...
The fallback is less accurate but ensures RepoAtlas produces useful output for any language.
...
Enable `repoatlas scan <github-url>` to score public repos without cloning.
...
> Generated by [RepoAtlas](https://github.com/repoatlas/repoatlas).
...
For detailed module documentation, see [.repoatlas/modules.md](.repoatlas/modules.md).
For dependency graph, see [.repoatlas/dependency-graph.json](.repoatlas/dependency-graph.json).
...
For full conventions documentation, see [.repoatlas/conventions.md](.repoatlas/conventions.md).
...
RepoAtlas should never crash on unexpected input. If a file can't be parsed, skip it and log a warning. If a language isn't supported, fall back to regex. If the repo structure is unusual, do best-effort analysis and note limitations in the output.
